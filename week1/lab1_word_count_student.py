...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda group : (group[0], sum(group[1])))
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda w : (w, 1)).reduceByKey(lambda v1, v2 : v1 + v2)
...
# TODO: Replace <FILL IN> with appropriate code
wordCountRDD = wordCount(shakeWordsRDD)
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, key=lambda pair : -pair[1])
